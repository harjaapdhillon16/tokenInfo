...
export const BasicTable = ({ tableData, onDeleteContact, data }) => {
	...
	const EditContact = async (item) => {
		try {
			const updateContact = await API.graphql(graphqlOperation(updateContact, item));
						...
						<tr>
							...
							<td className="dotaction   ">
								<Dropdown>
									...
									<Dropdown.Menu>
										<Dropdown.Item onClick={() => setOnEditContact(item)}>
											Edit
										</Dropdown.Item>

										<Dropdown.Item onClick={() => onDeleteContact(item.id)}>
											Delete
										</Dropdown.Item>
									</Dropdown.Menu>
								</Dropdown>
