...
const FormsScreen = () => {
	...
	const { user, agent, formItems, onFormItemsUpdate, onFormItemUnitUpdate } = useContext(
		AppContext
	);
					...
					<Col md={6} className="dashboardCards pt-5">
						<Dropdown>
							...
							<Dropdown.Menu>
								...
								<Dropdown.Item
									onClick={() => handleFilter('formName', 'REBNY COVID Liability Form')}>
					...
					filtered.map((item, i) => (
						<Row className=" border-bottom pb-3 mt-5 form-row " key={i}>
							...
							<Col md={4} className="text-right pr-0">
								...
								{item.status === 'SIGNED' && (
									<ShareButton itemData={item} item={item} itemUrl={item.id} />
								)}
							</Col>
						</Row>
					))
